# Edge Function: enviar-notificacion-tarea (con soporte para comentarios)

Copiar y pegar TODO este código en Supabase Dashboard → Edge Functions → `enviar-notificacion-tarea` → Reemplazar todo → Deploy.
...
interface NotificacionData {
  ...
  tipo: 'creacion' | 'cambio_estado' | 'comentario';
...
serve(async (req) => {
  ...
  try {
    ...
    const { tarea_id, tipo, estado_anterior, estado_nuevo, comentario, comentario_autor, comentario_autor_email } = body;
    ...
    if (tipo === 'comentario' && comentario_autor_email) {
      if (!destinatarios.includes(comentario_autor_email)) {
        destinatarios.push(comentario_autor_email);
      }
    }
    ...
    let asunto: string;
    if (tipo === 'creacion') {
      asunto = buildAsunto(tarea);
    } else if (tipo === 'comentario') {
      asunto = `Nuevo comentario - Tarea ${tarea.consecutivo}`;
    } else {
      asunto = `Cambio de Estado - Tarea ${tarea.consecutivo}`;
    }

    const html = generarHtmlCorreo(tarea, tipo, estado_anterior, estado_nuevo, comentario, comentario_autor);
    ...
    const mailOptions = {
      ...
      subject: asunto,
      html,
      text: tipo === 'comentario'
        ? `Tarea: ${tarea.consecutivo}\nComentario de: ${comentario_autor || 'N/A'}\nMensaje: ${comentario || ''}`
...
function generarHtmlCorreo(
  ...
  if (tipo === 'creacion') {
    tituloPrincipal = 'Nueva Solicitud de Trabajo';
  } else if (tipo === 'comentario') {
    tituloPrincipal = 'Nuevo comentario en tarea';
  } else {
    tituloPrincipal = 'Actualización de Tarea';
  }

  // ======= SECCIÓN COMENTARIO =======
  let seccionComentario = '';
  if (tipo === 'comentario' && comentario) {
    seccionComentario = `
    <tr>
      <td colspan="2" style="padding:0 0 24px;">
        <div style="background:#FFFBEB;border:2px solid #FCD34D;border-radius:12px;padding:20px;">
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:12px;">
            <div style="width:32px;height:32px;border-radius:50%;background:#F59E0B;display:flex;align-items:center;justify-content:center;">
              <span style="color:#fff;font-size:14px;font-weight:700;">${(comentario_autor || 'U').charAt(0).toUpperCase()}</span>
            </div>
            <div>
              <div style="font-size:13px;font-weight:700;color:#111827;">${comentario_autor || 'Usuario'}</div>
              <div style="font-size:10px;color:#9CA3AF;text-transform:uppercase;letter-spacing:1px;">Nuevo comentario</div>
            </div>
          </div>
          <div style="background:#fff;border-radius:8px;padding:16px;border:1px solid #FDE68A;">
            <p style="margin:0;font-size:14px;color:#374151;line-height:1.6;white-space:pre-wrap;">${comentario.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</p>
          </div>
        </div>
      </td>
    </tr>`;
    ...
    <tr>
      <td colspan="2" style="padding:0 0 24px;">
        <div style="background:#F9FAFB;border-radius:8px;padding:14px 16px;">
          <div style="font-size:10px;font-weight:700;color:#9CA3AF;letter-spacing:1.5px;text-transform:uppercase;margin-bottom:8px;">ESTADO ACTUAL</div>
  ...
  } else if (tipo === 'cambio_estado' && estado_anterior && estado_nuevo) {
    seccionEstado = `
    <tr>
      <td colspan="2" style="padding:0 0 24px;">
