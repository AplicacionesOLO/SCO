...
class TareaService {
  ...
  async updateTarea(id: string, data: UpdateTareaData): Promise<Tarea> {
    try {
      ...
      if (data.items) {
        ...
        // ── DEDUCCIÓN DE INVENTARIO (NO BLOQUEANTE) ──────────────────────
        // Wrapped in try/catch - si falla, la tarea se guarda igual pero se
        // registra el error en consola para diagnóstico. El popup vacío es
        // historia: ahora el error se muestra bien en el frontend.
        try {
          await this.deducirInventario(id, data.items, user.id);
        } catch (deduccionError) {
          console.error('[Inventario] Error en deducción (tarea guardada igual):', deduccionError);
          // Re-lanzamos para que el frontend muestre el popup con el mensaje real
          throw deduccionError;
        }
  ...
  // =====================================================
  // DEDUCCIÓN DE INVENTARIO (MÉTODO PRIVADO)
  // =====================================================

  /**
   * Deduce inventario para los items de una tarea.
   * Valida stock con filtro por tienda real (no solo RLS),
   * expande BOM de productos, y registra movimientos.
   * Lanza error descriptivo si el stock es insuficiente.
   */
  private async deducirInventario(
    tareaId: string,
    items: UpdateTareaData['items'],
    usuarioId: string
  ): Promise<void> {
    if (!items || items.length === 0) return;

    // Verificar si ya se descontó inventario para esta tarea
    const { data: existingMovs, error: existingError } = await supabase
      .from('inventario_movimientos')
      .select('id')
      .eq('referencia_tipo', 'tarea')
      .eq('referencia_id', tareaId)
      .limit(1);

    if (existingError) {
      console.error('[Inventario] Error verificando movimientos existentes:', existingError);
      return;
    }

    if (existingMovs && existingMovs.length > 0) {
      // Ya se descontó, no duplicar
      return;
    }

    // Obtener datos de la tarea para el consecutivo y tienda
    const { data: tareaData, error: tareaError } = await supabase
      .from('tareas')
      .select('consecutivo, tienda_id')
      .eq('id', tareaId)
      .single();

    if (tareaError || !tareaData) {
      console.error('[Inventario] Error obteniendo datos de tarea:', tareaError);
      return;
    }

    const consecutivo = tareaData.consecutivo || 'N/A';
    const tiendaId = tareaData.tienda_id;
    ...
    interface ArticuloAfectado {
      articulo_id: number;
      cantidad: number;
      on_hand_anterior: number;
    }

    const movimientos: MovimientoInventario[] = [];
    const articulosAfectados: ArticuloAfectado[] = [];

    for (const item of items) {
      // ── Artículo directo de inventario ──
      if (item.inventario_id && item.cantidad > 0) {
        try {
          const resultado = await this.validarYObtenerNivel(
            item.inventario_id,
            item.cantidad,
            tiendaId,
            item.descripcion || `artículo #${item.inventario_id}`
          );
          ...
          movimientos.push({
            ...
            tipo: 'venta',
            cantidad: -item.cantidad,
            stock_anterior: resultado.onHand,
            stock_posterior: resultado.nuevoOnHand,
          ...
          articulosAfectados.push({
            articulo_id: item.inventario_id,
            cantidad: item.cantidad,
            on_hand_anterior: resultado.onHand
          });
        } catch (err) {
          throw new Error(
      ...
      // ── Producto con BOM ──
        ...
        try {
          ...
          if (bomError) {
            console.error('[Inventario] Error expandiendo BOM para producto', item.producto_id, bomError);
            continue;
          }

          if (!bomItems || bomItems.length === 0) {
            console.warn('[Inventario] Producto sin BOM definido:', item.producto_id);
            continue;
          }
          ...
          for (const bomItem of bomItems) {
            ...
            const descripcionComp = bomItem.inventario?.descripcion_articulo
              || bomItem.inventario?.codigo_articulo
          ...
          throw new Error(
            `Error procesando producto "${item.descripcion || 'producto #' + item.producto_id}" en tarea #${consecutivo}. ` +
            (err instanceof Error ? err.message : '')
          );
        }
      }
    ...
    if (movError) {
      console.error('[Inventario] Error creando movimientos:', movError);
      throw new Error('Error al registrar movimientos de inventario: ' + movError.message);
    }
    ...
    for (const articulo of articulosAfectados) {
      ...
      try {
        ...
        await supabase
          ...
          }, { onConflict: 'articulo_id,tienda_id' });
      } catch (upsertError) {
        console.error('[Inventario] Error actualizando nivel para artículo', articulo.articulo_id, upsertError);
        // No lanzamos — el movimiento ya está registrado, el nivel se puede corregir después
      }
    }
  ...
  /**
   * Valida stock disponible y devuelve el nivel actual.
   * Filtra por tienda_id para evitar falsos "sin stock" por RLS.
   */
  private async validarYObtenerNivel(
    articuloId: number,
    cantidadRequerida: number,
    tiendaId: string,
    descripcion: string
  ): Promise<{ onHand: number; nuevoOnHand: number }> {
    const { data: nivelActual, error: nivelError } = await supabase
      .from('inventario_niveles')
    ...
    if (nivelError) {
      console.error('[Inventario] Error consultando nivel para artículo', articuloId, nivelError);
      throw new Error(`No se pudo consultar el stock de "${descripcion}". Error: ${nivelError.message}`);
    }

    const onHand = nivelActual?.on_hand || 0;
    const reservado = nivelActual?.reservado || 0;
    const disponible = onHand - reservado;
    ...
    return {
      onHand,
      nuevoOnHand: onHand - cantidadRequerida
    };
  }
